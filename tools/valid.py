...
import sys
sys.path.append('../')   # valid.py in tools/
...
def main():
    ...
    data_loader, test_dataset = make_test_dataloader(cfg)   # get dataset from xxxDataset
    ...
    for i, (images, annos) in enumerate(data_loader):
        ...
        # validation has no gradient propagation
        with torch.no_grad():
            final_heatmaps = None
            tags_list = []
    ...
    # evaluate in xxxDataset
    # hie has 'valid' and 'test' set, 'valid' has annotation, 'test' has no annotation
