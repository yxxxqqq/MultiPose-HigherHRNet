...
import sys
sys.path.append('../')   # dist_train.py in tools/
...
# the main worker to process training
def main_worker( gpu, ngpus_per_node, args, final_output_dir, tb_log_dir):
    ...
    if not cfg.MULTIPROCESSING_DISTRIBUTED or (
        ...
        # writer_dict['writer'].add_graph(model, (dump_input, ))   #解决训练报错问题
    ...
    loss_factory = MultiLossFactory(cfg).cuda()    # heatmap+ae: [heatmaps_losses, push_losses, pull_losses]

    # Data loading code
    train_loader = make_dataloader(   # get dataset from xxxKeypoints
    ...
    best_model = False
    last_epoch = -1
    optimizer = get_optimizer(cfg, model)   # SGD, Adam
    ...
    if cfg.AUTO_RESUME and os.path.exists(checkpoint_file):   # Continue to train using last checkpoint
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        perf_indicator = epoch   # bast_perf is the newest model, but not the best?
