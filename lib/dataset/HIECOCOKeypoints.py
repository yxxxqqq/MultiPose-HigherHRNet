...
import pycocotools
from .HIECOCODataset import HIECOCODataset
...
class HIECOCOKeypoints(HIECOCODataset):
    def __init__(self,
                 cfg,
                 dataset_name,
        ...
        if cfg.DATASET.WITH_CENTER:
            assert cfg.DATASET.NUM_JOINTS == 15, 'Number of joint with center for HIECOCO is 15'
        else:
            assert cfg.DATASET.NUM_JOINTS == 14, 'Number of joint for HIECOCO is 14'
    ...
    def get_joints(self, anno):
        ...
        for i, obj in enumerate(anno):
            keypoints=[int(k) for k in obj['keypoints']]   # keypoints in hie json is list with 'str' type element
            joints[i, :self.num_joints_without_center, :3] = \
                np.array(keypoints).reshape([-1, 3])     # obj['keypoints'] is a list with str type element
            if self.with_center:
