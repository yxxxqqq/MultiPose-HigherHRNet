...
def match_by_tag(inp, params):
    ...
    for i in range(params.num_joints):
        ...
        mask = joints[:, 2] > params.detection_threshold  # the confidence score should large the threshold(0.1)
        tags = tags[mask]
        joints = joints[mask]
        ...
        if i == 0 or len(joint_dict) == 0:
            ...
            diff_normed = np.linalg.norm(diff, ord=2, axis=2)   # L2
...
class HeatmapParser(object):
    ...
    def refine(self, det, tag, keypoints):
        ...
        for i in range(keypoints.shape[0]):
            ...
            # np.argmax(): get the index after flatten(expanded to a one-dimensional array)
            # np.unravel_index(): get the index of the corresponding original array.
