...
def get_outputs(
    ...
    if with_flip:
        ...
        heatmaps.append(outputs[-1][:, :cfg.DATASET.NUM_JOINTS])
        tags.append(outputs[-1][:, cfg.DATASET.NUM_JOINTS:])
        if 'coco_kpt' == cfg.DATASET.DATASET:
            dataset_name = 'COCO'
        elif 'crowd_pose_kpt' == cfg.DATASET.DATASET:
    ...
    if project2image and size_projected:
        heatmaps = [
            torch.nn.functional.interpolate(   # upsample
                hms,
...
def get_multi_stage_outputs(
    ...
    outputs = model(image)   # outputs: [batch_image, num_joints, height, width]
    for i, output in enumerate(outputs):
        if len(outputs) > 1 and i != len(outputs) - 1:
            output = torch.nn.functional.interpolate(   # upsample: bilinear interpolation
                output,
                size=(outputs[-1].size(2), outputs[-1].size(3)),  # height, width
                mode='bilinear',
                align_corners=False
            )
    ...
    if with_flip:
        ...
        for i in range(len(outputs_flip)):
            ...
            if len(outputs_flip) > 1 and i != len(outputs_flip) - 1:
                output = torch.nn.functional.interpolate(   # upsample
                    output,
