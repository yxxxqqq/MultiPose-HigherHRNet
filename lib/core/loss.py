...
class HeatmapLoss(nn.Module):
    ...
    def forward(self, pred, gt, mask):
        ...
        # mask: [height, width] of a person. expand the size as pred
        loss = ((pred - gt)**2) * mask[:, None, :, :].expand_as(pred)
        loss = loss.mean(dim=3).mean(dim=2).mean(dim=1)   # (torch.mean), the average of all elements, return a tensor
        # loss = loss.mean(dim=3).mean(dim=2).sum(dim=1)
        return loss

# "Associative Embedding:End-to-End Learning for Joint Detection and Grouping. NIPS'17"
class AELoss(nn.Module):
    def __init__(self, loss_type):
        super().__init__()
        self.loss_type = loss_type    # exponential
    ...
    def singleTagLoss(self, pred_tag, joints):
        ...
        tags = []
        pull = 0
        for joints_per_person in joints:   # iterate joints_per_person for all persons
            tmp = []
        ...
        A = tags.expand(*size)
        B = A.permute(1, 0)    # permute(change) the dimensions(0,1) of this tensor: 0,1 -> 1,0
        ...
        if self.loss_type == 'exp':
            ...
            push = torch.clamp(diff, min=0).sum() - num_tags  # clamp the value between min~max
        else:
...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_joints = cfg.MODEL.NUM_JOINTS  # 14 or 17
