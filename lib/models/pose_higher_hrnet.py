...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # newly added based on HRNet
        # pre_stage_channels[0], the output of last stage
    ...
    def _make_final_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            input_channels = deconv_cfg.NUM_CHANNELS[i]   # the channels of stem net output
            output_channels = cfg.MODEL.NUM_JOINTS + dim_tag \
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            if deconv_cfg.CAT_OUTPUT[i]:
                ...
                input_channels += final_output_channels   # concatenation
            ...
            for _ in range(cfg.MODEL.EXTRA.DECONV.NUM_BASIC_BLOCKS):  # There are 4 BasicBlocks
                layers.append(nn.Sequential(
    ...
    def forward(self, x):
        ...
        final_outputs = []
        x = y_list[0]   # the last stage-4 output
        y = self.final_layers[0](x)
        final_outputs.append(y)

        for i in range(self.num_deconvs):
            if self.deconv_config.CAT_OUTPUT[i]:
                x = torch.cat((x, y), 1)  # cat on dimension 1
